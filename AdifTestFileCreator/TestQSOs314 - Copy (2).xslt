...
etc., that occur in each new version of the ADIF Specification.

It uses features in Microsoft .NET Framework 4.8 and XSLT version 1.0 along with Microsoft and custom extension functions.
...
Change History:
  2024-09-23: Added support for ADIF Specification 3.1.4
-->
<xsl:stylesheet version="1.0"
  xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  xmlns:ms="urn:schemas-microsoft-com:xslt"
  xmlns:dt="urn:schemas-microsoft-com:datatypes"
  xmlns:ax="http://adif.org.uk/adiftests"
  xmlns:ex="http://adif.org.uk/adiftestexamples">

  <xsl:import href="AdifLib.xslt"/>
  <xsl:output method="text" encoding="UTF-8"/>

  <xsl:param    name="adifStyle"     select="'adi'"/>
  <xsl:variable name="programid"     select="'AdifRelease'"/>
  ...
  <xsl:template name="deletedOrReadOnly">
    ...
    <xsl:value-of select="ax:commentLine2(concat(
      '==== Not including ',
      $name,
      $valueText,
  ...
  <xsl:template name="arrlSectFromDxccCsv">
    ...
    <xsl:element name="{concat('Field',$cnt)}">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:value-of select="ax:record($fieldName, value[@name='Abbreviation'], 'CALL', ax:callForDxcc($dxcc), 'DXCC', $dxcc)"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:element>
  ...
  <xsl:template match="/">
    ...
    <xsl:value-of select="ax:initialize($adifStyle, true(), /adif)"/>
    <xsl:value-of select="ax:setOptions($fieldSeparator, $recordSeparator)"/>
    <xsl:value-of select="ax:bof()"/>
    ...
    <xsl:variable name="version306OrLater" select="ax:adifVersionInt() >= 306"/>
    ...
    <xsl:for-each select="adif/fields/record">
      ...
      <xsl:choose>
        <xsl:when test="$fieldName='ADIF_VER'">
          <xsl:value-of select="ax:field($fieldName, ax:adifVersion())"/>
        </xsl:when>

        <xsl:when test="$fieldName='CREATED_TIMESTAMP'">
          <xsl:value-of select="ax:field($fieldName, concat(ms:format-date(/adif/@created, 'yyyyMMdd'), ' ', ms:format-time(/adif/@created, 'HHmmss')))"/>
        </xsl:when>

        <xsl:when test="$fieldName='PROGRAMID'">
          <xsl:value-of select="ax:field($fieldName, $programid)"/>
        </xsl:when>

        <xsl:when test="$fieldName='PROGRAMVERSION'">
          <xsl:value-of select="ax:field($fieldName, '1.0.0')"/>
        </xsl:when>

        <xsl:when test="$fieldName='USERDEFn'">
          <xsl:value-of select="ax:userDefNField('RCVD_FROM_DX_CLUSTER',      'B',  1, '')"/>
          <xsl:value-of select="ax:userDefNField('MY_POWER_CATEGORY',         'E',  2, '{QRPP,QRP,QRO}')"/>
          <xsl:value-of select="ax:userDefNField('MY_TEMPERATURE_FAHRENHEIT', 'N',  3, '{-50:150}')"/>
          <xsl:value-of select="ax:userDefNField('MY_HEIGHT_ASL_FEET',        'N',  4, '')"/>
          <xsl:value-of select="ax:userDefNField('NEXT_QSL_POSTING_DATE',     'D',  5, '')"/>
          <xsl:value-of select="ax:userDefNField('NEXT_QSL_POSTING_TIME',     'T',  6, '')"/>
          <xsl:value-of select="ax:userDefNField('MY_AMP',                    'S',  7, '')"/>
          <xsl:value-of select="ax:userDefNField('MY_AMP_INTL',               'I',  8, '')"/>
          <xsl:value-of select="ax:userDefNField('QSO_TRANSCRIPT',            'M',  9, '')"/>
          <xsl:value-of select="ax:userDefNField('QSO_TRANSCRIPT_INTL',       'G', 10, '')"/>
          <xsl:value-of select="ax:userDefNField('REMOTE_STATION_LAT',        'L', 11, '')"/>
          <xsl:value-of select="ax:userDefNField('REMOTE_STATION_LON',        'L', 12, '')"/>
          <xsl:value-of select="ax:eoh()"/>

          <xsl:value-of select="ax:userDefField('REMOTE_STATION_LAT',        'S018 28.430')"/>
          <xsl:value-of select="ax:userDefField('REMOTE_STATION_LON',        'W021 22.892')"/>
          <xsl:value-of select="ax:userDefField('QSO_TRANSCRIPT_INTL',       'NAME FRED&#13;&#10;QTH ALENÇON&#13;&#10;73 ES GOOD DX')"/>
          <xsl:value-of select="ax:userDefField('QSO_TRANSCRIPT',            'NAME FRED&#13;&#10;QTH ALENCON&#13;&#10;73 ES GOOD DX')"/>
          <xsl:value-of select="ax:userDefField('MY_AMP_INTL',               '1 KW')"/>
          <xsl:value-of select="ax:userDefField('MY_AMP',                    '1 KW')"/>
          <xsl:value-of select="ax:userDefField('NEXT_QSL_POSTING_TIME',     '1500')"/>
          <xsl:value-of select="ax:userDefField('NEXT_QSL_POSTING_DATE',     '20231101')"/>
          <!-- Note that ADX user-defined field names must be uppercase; ax:userDefNfield and ax:userDefField automatically convert them when ADX is being generated. -->
          <xsl:value-of select="ax:userDefField('my_height_asl_feet',        '150.5')"/>
          <xsl:value-of select="ax:userDefField('My_Temperature_Fahrenheit', '66')"/>
          <xsl:value-of select="ax:userDefField('mY_poweR_categorY',         'QrpP')"/>
          <xsl:value-of select="ax:userDefField('RCVD_FROM_DX_CLUSTER',      'N')"/>
          <xsl:value-of select="ax:record()"/>

          <xsl:value-of select="ax:appField('USING_YAGI',         'Y',             $programid, 'B')"/>
          <xsl:value-of select="ax:appField('WORDS_PER_MINUTE',   '18',            $programid, 'N')"/>
          <xsl:value-of select="ax:appField('NEXT_SKED_DATE',     '20180101',      $programid, 'D')"/>
          <xsl:value-of select="ax:appField('NEXT_SKED_TIME_ON',  '2130',          $programid, 'T')"/>
          <xsl:value-of select="ax:appField('NEXT_SKED_TIME_OFF', '223000',        $programid, 'T')"/>
          <xsl:value-of select="ax:appField('MY_MORSE_KEY',       'Vibroplex Bug', $programid, 'S')"/>
          <xsl:value-of select="ax:appField('MY_MORSE_KEY_INTL',  'Vibroplex Bug', $programid, 'I')"/>
          <xsl:value-of select="ax:appField('WX',                 'Cloud scattered&#13;&#10;Light rain',
                                                                                   $programid, 'M')"/>
          <xsl:value-of select="ax:appField('WX_INTL',            'Cloud scattered&#13;&#10;Light rain',
                                                                                   $programid, 'G')"/>
          <xsl:value-of select="ax:appField('NEXT_LAT',           'S018 28.430',   $programid, 'L')"/>
          <xsl:value-of select="ax:appField('NEXT_LON',           'W021 22.892',   $programid, 'L')"/>
          <xsl:value-of select="ax:record(  'MODE',               'CW')"/>

          <!-- Application fields may be for another application, so the program ID can have different values in one file. -->
          <xsl:value-of select="ax:appField('SENT_TO_DX_CLUSTER', 'Y',             'Other',    'B')"/>
          <xsl:value-of select="ax:appField('WORDS_PER_MINUTE',   '14',            $programid, 'N')"/>
          <xsl:value-of select="ax:record(  'MODE',               'CW')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='AGE'">
          <xsl:value-of select="ax:record($fieldName,   '9')"/>
          <xsl:value-of select="ax:record($fieldName,  '57')"/>
          <xsl:value-of select="ax:record($fieldName, '102')"/>
        </xsl:when>

        <xsl:when test="$fieldName='ALTITUDE' or $fieldName='MY_ALTITUDE'">
          <xsl:value-of select="ax:record($fieldName, '-20')"/>
          <xsl:value-of select="ax:record($fieldName, '150.3')"/>
          <xsl:value-of select="ax:record($fieldName, '5000')"/>
        </xsl:when>        
        
        <xsl:when test="$fieldName='ANT_AZ'">
          <xsl:value-of select="ax:record($fieldName,   '0')"/>
          <xsl:value-of select="ax:record($fieldName, '108.3')"/>
          <xsl:value-of select="ax:record($fieldName, '360')"/>
        </xsl:when>

        <xsl:when test="$fieldName='ANT_EL'">
          <!-- This is an opportunity also to test handling of negative numbers. -->
          <xsl:value-of select="ax:record($fieldName, '-90')"/>
          <xsl:value-of select="ax:record($fieldName, '-35.5')"/>
          <xsl:value-of select="ax:record($fieldName,   '-.5')"/>
          <xsl:value-of select="ax:record($fieldName,  '-0.5')"/>
          <xsl:value-of select="ax:record($fieldName,   '0')"/>
          <xsl:value-of select="ax:record($fieldName,  '35.5')"/>
          <xsl:value-of select="ax:record($fieldName,  '90')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='ANT_PATH'">
          <xsl:for-each select="/adif/enumerations/enumeration[@name='Ant_Path']/record">
            <xsl:value-of select="ax:record($fieldName, value[@name='Abbreviation'])"/>
          </xsl:for-each>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='AWARD_SUBMITTED'">
          <xsl:if test="$version306OrLater">
            <xsl:value-of select="ax:record($fieldName, 'ADIF_CENTURY_BASIC,ADIF_CENTURY_SILVER,ADIF_SPECTRUM_100-160m')"/>
          </xsl:if>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='AWARD_GRANTED'">
          <xsl:if test="$version306OrLater">
            <xsl:value-of select="ax:record($fieldName, 'ADIF_CENTURY_BASIC,ADIF_CENTURY_SILVER,ADIF_SPECTRUM_100-160m')"/>
            <xsl:value-of select="ax:record($fieldName, 'ADIF_CENTURY_BASIC', 'AWARD_SUBMITTED', 'ADIF_CENTURY_BASIC,ADIF_CENTURY_SILVER,ADIF_SPECTRUM_100-160m')"/>
          </xsl:if>
        </xsl:when>

        <xsl:when test="$fieldName='A_INDEX'">
          <xsl:value-of select="ax:record($fieldName,   '0')"/>
          <xsl:value-of select="ax:record($fieldName, '250.5')"/>
          <xsl:value-of select="ax:record($fieldName, '400')"/>
        </xsl:when>        
        ...
        <xsl:when test="$fieldName='BAND' or $fieldName='BAND_RX'">
          ...
          <xsl:value-of select="ax:record($fieldName, '20M')"/>
          <xsl:value-of select="ax:record($fieldName, '70Cm')"/>
          <xsl:value-of select="ax:record($fieldName, '23CM')"/>
          <xsl:value-of select="ax:record($fieldName, '6Mm')"/>
          <xsl:value-of select="ax:record($fieldName, '4MM')"/>
        </xsl:when>

        <xsl:when test="$fieldName='CALL'">
          <xsl:value-of select="ax:record($fieldName, 'g5o')"/>
          <xsl:value-of select="ax:record($fieldName, 'G5o')"/>
          <xsl:value-of select="ax:record($fieldName, 'G2AAA/P')"/>
          <xsl:value-of select="ax:record($fieldName, 'MM/K6DF/P')"/>
          <xsl:value-of select="ax:record($fieldName, 'G2BBB/M')"/>
          <xsl:value-of select="ax:record($fieldName, 'G2CCC/MM')"/>
          <xsl:value-of select="ax:record($fieldName, 'K6DF/1')"/>

          <xsl:value-of select="ax:commentLine('This is a real call - see http://www.southgatearc.org/news/february2013/romanian_special_event_station_yo2013eyowf.htm')"/>
          <xsl:value-of select="ax:record($fieldName, 'YO2013EYOWF')"/>
        </xsl:when>

        <xsl:when test="$fieldName='CHECK'">
          <xsl:value-of select="ax:commentLine('For ARRL sweep stakes, the year of birth')"/>
          <xsl:value-of select="ax:record($fieldName, '{YEAR_OF_BIRTH(37)}')"/>
        </xsl:when>

        <xsl:when test="$fieldName='CLASS'">
          <xsl:value-of select="ax:record($fieldName, 'High power unassisted')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='CLUBLOG_QSO_UPLOAD_STATUS' or $fieldName='HAMLOGEU_QSO_UPLOAD_STATUS' or $fieldName='HAMQTH_QSO_UPLOAD_STATUS' or $fieldName='HRDLOG_QSO_UPLOAD_STATUS' or $fieldName='QRZCOM_QSO_UPLOAD_STATUS'">
          ...
          <xsl:for-each select="/adif/enumerations/enumeration[@name='QSO_Upload_Status']/record">
            <xsl:value-of select="ax:record($fieldName, value[@name='Status'])"/>
            <xsl:if test="value[@name='Status']!='N'">
              <xsl:value-of select="ax:record($fieldName, value[@name='Status'], concat($fieldPrefix, 'QSO_UPLOAD_DATE'), '{QSO_DATE_OFF+6.3}')"/>
            </xsl:if>
          </xsl:for-each>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='CNTY' or $fieldName='MY_CNTY'">
          ...
          <xsl:for-each select="/adif/enumerations/enumeration[@name='Secondary_Administrative_Subdivision']/record">
            ...
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:value-of select="ax:record(
                    'CALL',                       ax:callForPrimaryAdministrativeSubdivision($dxcc, substring-before($code, ',')),
          ...
          <xsl:value-of select="ax:commentLine('CNTY values except for U.S. Counties are held outside the ADIF specification')"/>          
          <xsl:value-of select="ax:record(
            'CALL',                       ax:callForDxcc($usaDxcc),
        ...
        <xsl:when test="$fieldName='CONT'">
          <xsl:for-each select="/adif/enumerations/enumeration[@name='Continent']/record">
            ...
            <xsl:value-of select="ax:record('CALL', ax:callForCont($abbreviation), $fieldName, $abbreviation)"/>
          </xsl:for-each>
        </xsl:when>

        <xsl:when test="$fieldName='CONTACTED_OP'">
          <!-- Ensure the two calls are not the same by using difference DXCCs. -->
          <xsl:value-of select="ax:record($fieldName, ax:callForDxcc($canadaDxcc), 'CALL', ax:callForDxcc($usaDxcc))"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='CONTEST_ID'">
          <xsl:for-each select="/adif/enumerations/enumeration[@name='Contest_ID']/record">
            ...
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:value-of select="ax:record($fieldName, $contestId, 'BAND', ax:bandForContest($contestId))"/>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:for-each>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='CQZ'">
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($canadaDxcc), $fieldName, '{}')"/>
        </xsl:when>
          ...
          <xsl:for-each select="/adif/enumerations/enumeration[@name='Credit']/record">
            <xsl:choose>
              ...
              <xsl:otherwise>
                ...
                <xsl:value-of select="ax:record($fieldName, concat(value[@name='Credit For'], ':', $qslmedium))"/>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:for-each>

          <!-- Test with one Credit value alone. -->
          <xsl:value-of select="ax:record($fieldName, 'DXCC')"/>

          <!-- Test with one Credit value and one QSL Medium. -->
          <xsl:value-of select="ax:record($fieldName, 'DXCC:CARD')"/>

          <!-- Test with one Credit value and two QSL Medium. -->
          <xsl:value-of select="ax:record($fieldName, 'DXCC_MODE:CARD&amp;LOTW')"/>

          <!-- Test with two Credit values alone. -->
          <xsl:value-of select="ax:record($fieldName, 'CQDX,IOTA')"/>

          <!-- Test with two Credit values and one QSL Medium. -->
          <xsl:value-of select="ax:record($fieldName, 'CQDX_MODE:CARD,CQDX_BAND:CARD')"/>

          <!-- Test with two Credit values and two QSL Medium. -->
          <xsl:value-of select="ax:record($fieldName, 'CQDX_MOBILE:CARD,CQDX_BAND:EQSL&amp;CARD')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='DARC_DOK'">
          <xsl:if test="$version306OrLater">
            <!-- Three examples are a standard one, a special one with all letters and the longest current special one. -->
            <xsl:value-of select="ax:record('CALL', ax:callForDxcc($germanyDxcc), $fieldName, 'A01')"/>
            <xsl:value-of select="ax:record('CALL', ax:callForDxcc($germanyDxcc), $fieldName, 'ILERA')"/>
            <xsl:value-of select="ax:record('CALL', ax:callForDxcc($germanyDxcc), $fieldName, '50ESKILSTUNA')"/>
          </xsl:if>
        </xsl:when>

        <xsl:when test="$fieldName='DISTANCE'">
          <xsl:value-of select="ax:record($fieldName,    '0.0')"/>
          <xsl:value-of select="ax:record($fieldName,  '210')"/>
          <xsl:value-of select="ax:record($fieldName, '3000.5')"/>
        </xsl:when>
            ...
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:value-of select="ax:record(
                  ...
                  'CALL',     ax:callForDxcc(value[@name='Entity Code']))"/>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:for-each>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='EMAIL'">
          <xsl:value-of select="ax:record($fieldName, 'someone@somewhere.com')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='EQSL_QSLRDATE' or $fieldName='LOTW_QSLRDATE'or $fieldName='QSLRDATE'"/>

        <!-- No test QSOs because these fields are covered by the test of xxxQSL_SENT -->
        <xsl:when test="$fieldName='EQSL_QSLSDATE' or $fieldName='LOTW_QSLSDATE' or $fieldName='QSLSDATE'"/>

        <xsl:when test="$fieldName='EQSL_QSL_RCVD' or $fieldName='LOTW_QSL_RCVD' or $fieldName='QSL_RCVD'">
            ...
            <xsl:choose>
              ...
              <xsl:otherwise>
                ...
                <xsl:if test="$status='Y' or $status='I' or $status='V'">
                  <xsl:value-of select="ax:record($fieldName, value[@name='Status'], concat($fieldPrefix, 'QSLRDATE'),  '{QSO_DATE_OFF+3}')"/>
                </xsl:if>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:for-each>
        ...
        <xsl:when test="$fieldName='EQSL_QSL_SENT' or $fieldName='LOTW_QSL_SENT' or $fieldName='QSL_SENT'">
            ...
            <xsl:choose>
              ...
              <xsl:otherwise>
                ...
                <xsl:if test="$status='Y' or $status='Q' or $status='I'">
                  <xsl:value-of select="ax:record($fieldName, value[@name='Status'], concat($fieldPrefix, 'QSLSDATE'), '{QSO_DATE_OFF+3}')"/>
                </xsl:if>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:for-each>
        ...
        <xsl:when test="$fieldName='FISTS'">
          <xsl:value-of select="ax:record($fieldName,     '1', 'CALL', 'GX0IPX')"/>
          <xsl:value-of select="ax:record($fieldName,  '8385', 'CALL', 'G3ZOD')"/>
          <xsl:value-of select="ax:record($fieldName, '08385', 'CALL', 'G3ZOD')"/>
        </xsl:when>

        <xsl:when test="$fieldName='FISTS_CC'">
          <xsl:value-of select="ax:record($fieldName,  '9876', 'CALL', 'G0QQQ')"/>
          <xsl:value-of select="ax:record($fieldName, '10876', 'CALL', 'M0QQQ')"/>
        </xsl:when>

        <xsl:when test="$fieldName='FORCE_INIT'">
          <xsl:for-each select="document('')//ex:booleanValues/ex:boolean/@value">
            <xsl:value-of select="ax:record($fieldName, ., 'BAND', '2m')"/>
          </xsl:for-each>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='FREQ' or $fieldName='FREQ_RX'">
          ...
          <xsl:for-each select="/adif/enumerations/enumeration[@name='Band']/record">
            ...
            <xsl:value-of select="ax:record($fieldName, $freqLo)"/>
            <xsl:value-of select="ax:record($fieldName, $freqLo, concat('BAND', $fieldSuffix), '{}')"/>
            <xsl:value-of select="ax:record($fieldName, $freqHi, concat('BAND', $fieldSuffix), '{}')"/>
          </xsl:for-each>
        </xsl:when>

        <xsl:when test="$fieldName='GRIDSQUARE' or $fieldName='MY_GRIDSQUARE'">
          <xsl:variable name="call" select="ax:callForDxcc($englandDxcc)"/>
          <xsl:value-of select="ax:record($fieldName, 'IO',       'CALL', $call)"/>
          <xsl:value-of select="ax:record($fieldName, 'IO83',     'CALL', $call)"/>
          <xsl:value-of select="ax:record($fieldName, 'io83wj',   'CALL', $call)"/>
          <xsl:value-of select="ax:record($fieldName, 'IO83WJ',   'CALL', $call)"/>
          <xsl:value-of select="ax:record($fieldName, 'IO83WJ16', 'CALL', $call)"/>
          <xsl:value-of select="ax:record($fieldName, 'IO83WJ16', 'CALL', $call, concat($fieldName, '_EXT'), 'xa')"/>
          <xsl:value-of select="ax:record($fieldName, 'IO83WJ16', 'CALL', $call, concat($fieldName, '_EXT'), 'XA09')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='IOTA' or $fieldName='MY_IOTA'">
          ...
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($englandDxcc),  $fieldName, 'EU-005')"/>
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($englandDxcc),  $fieldName, 'eu-005', $fieldIotaIslandId, '00009292')"/>
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($walesDxcc),    $fieldName, 'EU-005', $fieldIotaIslandId, '9311')"/>
        </xsl:when>

        <xsl:when test="$fieldName='IOTA_ISLAND_ID' or $fieldName='MY_IOTA_ISLAND_ID'">
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($scotlandDxcc), $fieldName, '00011164')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='LAT' or $fieldName='MY_LAT'">
          ...
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($englandDxcc),                concat($fieldPrefix, 'LAT'), 'N052 43.432', concat($fieldPrefix, 'LON'), 'W002 07.500')"/>
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($englandDxcc),                concat($fieldPrefix, 'LON'), 'E000 47.345', concat($fieldPrefix, 'LAT'), 'N053 23.733')"/>
          <xsl:value-of select="ax:record('CALL', concat(ax:callForDxcc($englandDxcc), '/MM'), concat($fieldPrefix, 'LAT'), 'S037 09.337', concat($fieldPrefix, 'LON'), 'E028 54.085')"/>
          <xsl:value-of select="ax:record('CALL', concat(ax:callForDxcc($englandDxcc), '/MM'), concat($fieldPrefix, 'LAT'), 'n012 38.777', concat($fieldPrefix, 'LON'), 'w037 12.110')"/>
          <xsl:value-of select="ax:record('CALL', concat(ax:callForDxcc($englandDxcc), '/MM'), concat($fieldPrefix, 'LAT'), 's018 28.430', concat($fieldPrefix, 'LON'), 'w021 22.892')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='MAX_BURSTS'">
          <xsl:value-of select="ax:record($fieldName, '0')"/>
          <xsl:value-of select="ax:record($fieldName, '0.4')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='MODE'">
          <xsl:for-each select="/adif/enumerations/enumeration[@name='Mode']/record">
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:value-of select="ax:record($fieldName, value[@name='Mode'])"/>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:for-each>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='MS_SHOWER'">
          ...
          <xsl:for-each select="document('')//ex:showers/ex:shower/@value">
            <xsl:value-of select="ax:record($fieldName, ., 'BAND', '6m')"/>
          </xsl:for-each>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='MY_DXCC'">
          <xsl:for-each select="/adif/enumerations/enumeration[@name='DXCC_Entity_Code']/record">
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:value-of select="ax:record($fieldName, value[@name='Entity Code'])"/>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:for-each>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='MY_FISTS'">
          <xsl:value-of select="ax:record($fieldName,     '1')"/>
          <xsl:value-of select="ax:record($fieldName,  '8385')"/>
          <xsl:value-of select="ax:record($fieldName, '08385')"/>
        </xsl:when>

        <xsl:when test="$fieldName='MY_ITU_ZONE'">
          <xsl:value-of select="ax:record($fieldName, '2')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='MY_POTA_REF' or $fieldName='POTA_REF'">
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($usaDxcc), $fieldName, 'K-0059')"/>
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($usaDxcc), $fieldName, 'K-10000' )"/>
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($usaDxcc), $fieldName, 'K-0817,K-4578@US-WY,K-4566,K-4576,K-4573' )"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='MY_SOTA_REF' or $fieldName='SOTA_REF'">
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($usaDxcc),     $fieldName, 'W2/WE-003')"/>
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($englandDxcc), $fieldName, 'G/LD-003' )"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='MY_STATE' or $fieldName='STATE'">
          ...
          <xsl:for-each select="/adif/enumerations/enumeration[@name='Primary_Administrative_Subdivision']/record">
            ...
            <xsl:choose>
              ...
              <xsl:otherwise>
                <xsl:value-of select="ax:record(
                    'CALL',                       ax:callForPrimaryAdministrativeSubdivision($dxcc, $code),
                    concat($fieldPrefix, 'DXCC'), $dxcc,
                    $fieldName,                   $code)"/>
              </xsl:otherwise>
            </xsl:choose>
        ...
        <xsl:when test="$fieldName='MY_USACA_COUNTIES' or $fieldName='USACA_COUNTIES'">
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($usaDxcc), $fieldName, 'MA,Franklin:MA,Hampshire')"/>
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($usaDxcc), $fieldName, 'ma,hampshire:ma,franklin')"/>
        </xsl:when>

        <xsl:when test="$fieldName='MY_VUCC_GRIDS' or $fieldName='VUCC_GRIDS'">
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($usaDxcc), $fieldName, 'EN98,FM08,EM97,FM07')"/>
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($usaDxcc), $fieldName, 'fm07,em97,fm08,en98')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='NR_BURSTS'">
          <xsl:value-of select="ax:record($fieldName,   '0')"/>
          <xsl:value-of select="ax:record($fieldName,   '3')"/>
          <xsl:value-of select="ax:record($fieldName, '003')"/>
        </xsl:when>

        <xsl:when test="$fieldName='NR_PINGS'">
          <xsl:value-of select="ax:record($fieldName,   '0')"/>
          <xsl:value-of select="ax:record($fieldName,   '5')"/>
          <xsl:value-of select="ax:record($fieldName, '005')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='PFX'">
          <xsl:value-of select="ax:record('CALL', 'K1AAI', $fieldName, 'K1')"/>
          <xsl:value-of select="ax:record('CALL', 'W1AAJ', $fieldName, 'w1')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='QSL_RCVD_VIA' or $fieldName='QSL_SENT_VIA'">
          <xsl:for-each select="/adif/enumerations/enumeration[@name='QSL_Via']/record">
            ...
            <xsl:when test="value[@name='Import-only' or @name='Deleted']">
              ...
              <xsl:otherwise>
                <xsl:value-of select="ax:record($fieldName, value[@name='Via'])"/>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:for-each>
          <!-- Add a QSO with a value in lowercase. -->
          <xsl:value-of select="ax:record($fieldName, 'e')"/>
        </xsl:when>

        <xsl:when test="$fieldName='QSL_VIA'">
          <xsl:value-of select="ax:record($fieldName, 'ARRL QSL Bureau')"/>
        </xsl:when>

        <xsl:when test="$fieldName='QSO_COMPLETE'">
          <xsl:for-each select="/adif/enumerations/enumeration[@name='QSO_Complete']/record">
            <xsl:variable name="abbreviation" select="value[@name='Abbreviation']"/>
            <xsl:value-of select="ax:record($fieldName, $abbreviation)"/>
            <xsl:value-of select="ax:record($fieldName, translate($abbreviation, 'YNIL', 'ynil'))"/>
          </xsl:for-each>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='QSO_DATE'">
          <!-- QSO_DATE, TIME_ON and TIME_OFF already appear in every QSO with numerous values,
               ...
               If the ax:record and  ax:field calls include any of the four date/time fields, they will update the default QSO's
          ...
          <xsl:value-of select="ax:saveQsoStartEnd()"/>
          
          <xsl:value-of select="ax:record('QSO_DATE', '20230501',                             'TIME_ON', '2350', 'TIME_OFF', '0005')"/>
          <xsl:value-of select="ax:record('QSO_DATE', '20230502', 'QSO_DATE_OFF', '20230502', 'TIME_ON', '2350', 'TIME_OFF', '2355')"/>
          <xsl:value-of select="ax:record('QSO_DATE', '20230502', 'QSO_DATE_OFF', '20230503', 'TIME_ON', '2350', 'TIME_OFF', '0005')"/>

          <xsl:value-of select="ax:restoreQsoStartEnd()"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='QSO_RANDOM'">
          <xsl:for-each select="document('')//ex:booleanValues/ex:boolean/@value">
            <xsl:value-of select="ax:record($fieldName, .)"/>
          </xsl:for-each>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='QTH'">
          <!-- Include both _INTL and non-_INTL fields in one record because just the appropriate one will be output to the ADI or ADX file. -->
          <xsl:value-of select="ax:record(
            'CALL',                      ax:callForDxcc($franceDxcc),
            $fieldName,                  'Alencon',
            concat($fieldName, '_INTL'), 'Alençon')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='RST_RCVD'">
          ...
          <xsl:for-each select="document('')//ex:qsoReports/ex:qsoReport">
            <xsl:value-of select="ax:record('MODE', @mode, 'BAND', '6m', 'RST_RCVD', @value)"/>
            <xsl:value-of select="ax:record('MODE', @mode, 'BAND', '6m',                     'RST_SENT', @value)"/>
            <xsl:value-of select="ax:record('MODE', @mode, 'BAND', '6m', 'RST_RCVD', @value, 'RST_SENT', @value)"/>
          </xsl:for-each>
        </xsl:when>
          ...
          <xsl:if test="$version306OrLater">
            <xsl:for-each select="/adif/enumerations/enumeration[@name='Region']/record">
              ...
              <xsl:variable name="dxcc">
                <xsl:choose>
                  <xsl:when test="value[@name='Region Entity Code']='NONE'"><xsl:value-of select="'{DXCC}'"/></xsl:when>
                  <xsl:otherwise><xsl:value-of select="value[@name='DXCC Entity Code']"/></xsl:otherwise>
                </xsl:choose>
              </xsl:variable>              
              <xsl:variable name="call">
                <xsl:choose>
                  <xsl:when test="$region='NONE'">{CALL}</xsl:when>
                  <xsl:when test="$region='IV'"  >4U1V</xsl:when>
                  <xsl:when test="$region='AI'"  >IG9ZZA</xsl:when>
                  <xsl:when test="$region='SY'"  >IT9ZZB</xsl:when>
                  <xsl:when test="$region='BI'"  >JW0ZZC/B</xsl:when>
                  <xsl:when test="$region='SI'"  >GM0ZZD/S</xsl:when>
                  <xsl:when test="$region='KO'"  >Z6ZZE</xsl:when>
                  <xsl:when test="$region='ET'"  >TA1ZZF</xsl:when>
        ...
        <xsl:when test="$fieldName='RX_PWR'">
          <!-- Milliwatts are representd by digits to the right of the decimal point, e.g. 100 milliwatts can be .1
          ...
          <xsl:for-each select="document('')//ex:qsoPowers/ex:qsoPower">
            <xsl:value-of select="ax:record('RX_PWR', @value                  )"/>
            <xsl:value-of select="ax:record(                  'TX_PWR', @value)"/>
            <xsl:value-of select="ax:record('RX_PWR', @value, 'TX_PWR', @value)"/>
          </xsl:for-each>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='SFI'">          
          ...
          <xsl:value-of select="ax:record($fieldName,  '63')"/>
          <xsl:value-of select="ax:record($fieldName, '123')"/>
          <xsl:value-of select="ax:record($fieldName, '300')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='SKCC'">
          <xsl:value-of select="ax:record($fieldName,     '1')"/>
          <xsl:value-of select="ax:record($fieldName, '98765')"/>
          <xsl:value-of select="ax:record($fieldName, '98766C')"/>
          <xsl:value-of select="ax:record($fieldName, '98767T')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='SRX'">
          ...
          <xsl:value-of select="ax:record('SRX',    '1')"/>
          <xsl:value-of select="ax:record('SRX',  '002')"/>
          <xsl:value-of select="ax:record('SRX', '1003')"/>
          <xsl:value-of select="ax:record(               'STX',    '1')"/>
          <xsl:value-of select="ax:record(               'STX',  '002')"/>
          <xsl:value-of select="ax:record(               'STX', '1003')"/>
          <xsl:value-of select="ax:record('SRX',  '034', 'STX',  '143', 'SRX_STRING', 'SK')"/>
          <xsl:value-of select="ax:record('SRX',  '034', 'STX',  '143', 'SRX_STRING', 'SK', 'STX_STRING', 'WA')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='STATION_CALLSIGN'">
          <xsl:variable name="call1" select="ax:callForDxcc($englandDxcc)"/>
          <xsl:variable name="call2" select="ax:callForDxcc($canadaDxcc)"/>
          <xsl:variable name="call3" select="ax:callForDxcc($usaDxcc)"/>
          <xsl:value-of select="ax:record(                                        'OWNER_CALLSIGN', $call3)"/>
          <xsl:value-of select="ax:record(                    'OPERATOR', $call2)"/>
          <xsl:value-of select="ax:record(                    'OPERATOR', $call2, 'OWNER_CALLSIGN', $call3)"/>
          <xsl:value-of select="ax:record($fieldName, $call1)"/>
          <xsl:value-of select="ax:record($fieldName, $call1,                     'OWNER_CALLSIGN', $call3)"/>
          <xsl:value-of select="ax:record($fieldName, $call1, 'OPERATOR', $call2)"/>
          <xsl:value-of select="ax:record($fieldName, $call1, 'OPERATOR', $call2, 'OWNER_CALLSIGN', $call3)"/>
          <!-- More typically, all three fields will have the same value, so include a record with the same one. -->
          <xsl:value-of select="ax:record($fieldName, $call1, 'OPERATOR', $call1, 'OWNER_CALLSIGN', $call1)"/>
        </xsl:when>
          ...
          <xsl:for-each select="/adif/enumerations/enumeration[@name='Submode']/record">
            ...
            <xsl:choose>
              <xsl:when test="starts-with($submode,'JT9-')">
                <xsl:value-of select="ax:commentLine2(
                  concat('==== Not including ',
                  $submode,
                  ' because it is not supported by current software'))"/> 
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="ax:record($fieldName, $submode, 'MODE', value[@name='Mode'])"/>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:for-each>
          <!-- Include one with other than uppercase2 -->
          <xsl:value-of select="ax:record($fieldName, 'Usb', 'MODE', 'Ssb')"/>
          <!-- SUBMODE values do not have to be in the Submode enumeration -->
          <xsl:value-of select="ax:record($fieldName, 'DSB', 'MODE', 'AM')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='TEN_TEN'">
          <xsl:value-of select="ax:record($fieldName,     '1')"/>
          <xsl:value-of select="ax:record($fieldName, '00001')"/>
          <xsl:value-of select="ax:record($fieldName, '98765')"/>
        </xsl:when>
        ...
        <xsl:when test="$fieldName='WEB'">
          <xsl:value-of select="ax:record($fieldName, 'http://adif.org.uk')"/>
          <xsl:value-of select="ax:record($fieldName, 'http://adif.org.uk/')"/>
          <xsl:value-of select="ax:record($fieldName, 'http://adif.org.uk/305/ADIF_305.htm#QSO_Fields')"/>
        </xsl:when>

        <xsl:when test="$fieldName='WWFF_REF' or $fieldName='MY_WWFF_REF'">
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($usaDxcc),     $fieldName, 'KFF-0070')"/>
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($englandDxcc), $fieldName, 'GFF-0010')"/>
          <xsl:value-of select="ax:record('CALL', ax:callForDxcc($fijiDxcc),    $fieldName, '3D2FF-0006')"/>
        </xsl:when>

        <xsl:otherwise>
          <xsl:value-of select="ax:commentLine2(concat('**** Untested field name ', value[@name]))"/>
        </xsl:otherwise>

      </xsl:choose>
    </xsl:for-each>

    <xsl:value-of select="ax:commentReport(true())"/>
    <xsl:value-of select="ax:eof()"/>
  </xsl:template>
</xsl:stylesheet>